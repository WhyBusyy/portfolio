...
export const projects: ProjectData[] = [
  ...
  // ─── 제품 개발 ───
  {
    ...
    overview: {
      lead: "담당자 교체와 부분 리뉴얼이 겹쳐 레거시 스타일과 현행 스타일이 한 화면에 섞여 있었습니다. 디자인 측이 정의한 토큰 체계를 코드로 구현하고 공용 컴포넌트로 화면 전반을 통일한 뒤, 정합 규칙을 만들어 이후에도 디자인 시스템을 지키며 유지보수할 수 있게 했습니다.",
      detail:
        "3가지 문제가 있었습니다. 레거시 스타일과 현행 스타일이 한 화면에 섞여 있었고, 공용 컴포넌트는 일부 화면에서만 쓰였고, 색은 의미 토큰 대신 프리미티브 팔레트를 직접 참조하고 있었습니다. 원인은 담당자 교체였습니다. 디자인 시스템을 맡은 웹디자이너가 한 번 바뀌었고, 새 담당자가 팔레트 계층까지만 구현한 상태에서 일부 페이지만 리뉴얼이 진행된 것입니다. 이후 디자인 측이 역할 기반 토큰 체계를 정리했고, 저는 이를 코드로 구현해 화면 전반에 적용한 뒤 정합 규칙을 만들어 이후 UI 수정도 디자인 시스템을 지키며 진행할 수 있게 했습니다.",
    },
    screenshots: [],
    challenges: [
      {
      ...
      },
      {
        ...
        before: [
          "넘어온 디자인 시스템이 색상 팔레트 목록까지만 구현된 상태였음",
