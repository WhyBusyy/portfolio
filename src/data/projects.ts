...
export const projects: ProjectData[] = [
  ...
  // ─── 제품 개발 ───
  {
    ...
    overview: {
      lead: "여러 관리자가 같은 데이터를 단계별로 승인하는 어드민 화면을 재설계했습니다. 흐름마다 흩어져 있던 모달 6개를 단일 통합 모달로 합치고, 동시 편집 충돌과 재연결 중간 실패를 프론트·서버 양쪽에서 방어했습니다.",
      detail:
        "등록·수정·승인이 각각 다른 화면과 모달로 갈라져 있어 같은 검색·선택 UI가 여러 곳에 중복 구현돼 있었고, 한 곳을 고치면 나머지가 어긋나는 상태였습니다. 승인 담당자가 여럿이라 동시 편집 충돌도 잦았습니다. 화면을 구현하면서 응답 필드와 정렬 규칙까지 담은 API 명세를 직접 작성하고, 필요한 서버 엔드포인트도 함께 구현했습니다.",
    },
    screenshots: [],
    challenges: [
      {
        title: "흐름마다 따로 만들어진 모달 6개",
        before: [
          "신규 등록, 기존 항목 선택, 수정, 승인 등 흐름마다 모달이 따로 존재",
        ...
        after: [
          ...
          "어드민에서 먼저 완성한 뒤 같은 컴포넌트를 다른 화면으로 포팅해 UI 일관성 확보",
        ],
      },
      {
        title: "상세 진입 시 API를 4번 호출",
        before:
          "화면 하나를 그리는 데 연관 데이터를 각각 조회해 네 번의 왕복이 필요",
        after:
          "연관 데이터를 한 번에 내려주는 단일 조회 엔드포인트를 설계해 호출을 4회에서 1회로 축소하고, 하위 섹션은 개별 조회 대신 전달받은 데이터를 사용하도록 변경",
      },
      {
        title: "여러 관리자가 같은 데이터를 동시에 편집",
        before:
          "승인 담당자가 여럿이라, 한 사람이 보고 있는 사이 다른 사람이 바꾼 내용을 모른 채 덮어쓸 수 있는 구조",
        after: [
          "조회 시점의 최종 수정 시각을 함께 받아 수정 요청에 되실어 보내는 낙관적 잠금 방식 도입",
          "서버는 그 시각이 현재 값과 일치할 때만 반영하고, 변경이 감지되면 프론트에서 안내 모달을 띄우고 이동을 차단",
        ],
      },
      {
        title: "재연결 중간에 실패하면 데이터가 어중간하게 남음",
        before:
          "승인자가 잘못된 연결을 바로잡으려면 기존 연결을 끊고 새로 이어야 하는데, 중간에 실패하면 어느 쪽에도 속하지 않은 상태로 남을 위험",
        after: [
          "'재연결 → 기존 해제' 순서를 고정해, 앞 단계가 실패하면 원래 상태가 그대로 남아 그대로 재시도할 수 있도록 설계",
    ...
    metrics: [
      ...
      {
        value: "동시 편집 방어",
        change: "낙관적 잠금 + 멱등 재시도",
        label: "충돌·중간 실패 시 데이터 정합성 보장",
      },
    ],
  },
  {
