...
export const projects: ProjectData[] = [
  ...
  },
  {
    ...
    overview: {
      ...
      detail:
        "처음에는 이메일 알림을 만들어 붙였습니다. 쓰다 보니 한계가 분명했습니다. 적용 대상이 단일 서비스뿐이었고, 메일 본문이라 무엇이 왜 배포됐는지 한눈에 들어오지 않았습니다. 직전 성공 배포 이후의 커밋을 티켓별로 묶어 Slack에 보내는 Composite Action으로 다시 만들어 여러 서비스에 공통 적용하고, 배포 기록도 Notion에 자동으로 남게 했습니다.",
    },
    screenshots: [],
    challenges: [
      {
        ...
        before: [
          ...
          "먼저 만들어 둔 이메일 알림은 단일 서비스에만 적용돼 있었고, 본문이 길어 변경점이 눈에 들어오지 않음",
        ],
        after: [
      ...
      },
      {
        ...
        after:
          "운영 배포 워크플로 전반에 Notion 배포 기록을 자동 생성하는 액션을 배선하고, 앞서 만들었던 이메일 알림은 정리",
      },
    ],
    metrics: [
      {
  ...
  // ─── 아키텍처 · 성능 ───
  {
    ...
    overview: {
      lead: "수백 개 파일 규모의 프론트엔드 구조를 재설계했습니다. 후보 4안을 비교한 뒤, 의존성 전수조사 결과를 근거로 교과서적인 정답(완전한 Vertical Slice)을 채택하지 않고 하이브리드 구조를 택했습니다.",
      detail:
    ...
    decisions: [
      {
        question: "레이어 기준으로 나눌 것인가, 라우트 기준으로 모을 것인가",
        basis:
  ...
  },
  {
    ...
    decisions: [
      {
        question: "관리자 앱을 교체할 때, 에디터 후보를 다시 평가할 것인가",
        basis:
          "설치되는 의존성은 실제로 쓰지 않는 플러그인까지 포함한 풀 메타패키지였고, 정작 런타임에 쓰이는 산출물은 레포에 커밋된 1.2MB 번들 하나뿐이었습니다. 교체 대상은 명확했고, 남은 판단은 무엇으로 바꿀지였습니다.",
        options: [
          {
            label: "후보를 새로 비교해 최적안 선정",
            verdict: "기각",
            reason:
        ...
        outcome:
          "이식 과정에서 선례 쪽에도 남아 있던 폰트 크기 유실 결함을 통합 테스트로 발견해 함께 고쳤습니다. 의존성은 약 1.8GB 줄었습니다.",
      },
    ],
    challenges: [
      {
  ...
  // ─── 제품 개발 ───
  {
    ...
    overview: {
      lead: "담당자 변경과 디자인 리뉴얼이 겹치면서 페이지와 컴포넌트마다 적용된 스타일 기준이 달랐습니다. 디자인 측이 정의한 토큰 체계를 코드로 구현하고, 따로 마크업돼 있던 요소를 공용 컴포넌트로 대체한 뒤, 정합 규칙을 만들어 이후에도 디자인 시스템을 지키며 유지보수할 수 있게 했습니다.",
      detail:
    ...
    challenges: [
      ...
      {
        title: "공용 컴포넌트가 일부에만 쓰이고 나머지는 따로 마크업",
        before:
      ...
      {
        title: "색을 의미 토큰이 아니라 팔레트에서 직접 참조",
        before: [
    ...
    metrics: [
      ...
      {
        value: "컴포넌트 대체",
        change: "따로 마크업 → 공용 컴포넌트",
        label: "전체 페이지에 적용",
      },
      {
        value: "유지보수 기준",
        change: "정합 규칙 문서화",
        label: "이후 UI 수정도 디자인 시스템을 지키며 진행",
      },
    ],
  },
  {
    ...
    challenges: [
      {
        ...
        before:
          "웨비나를 서비스 안에서 바로 시청할 수 있게 하려면 외부 스트리밍 서비스를 연동해야 하는 상황",
        after:
  ...
  },
  {
    ...
    decisions: [
      {
        question: "CMS를 직접 만들 것인가, 외부 서비스를 붙일 것인가",
        basis:
          "이미 운영 중인 서버와 DB, 인증, 파일 스토리지가 있었습니다. CMS에 필요한 기반은 대부분 이미 갖춰져 있는 상태였습니다.",
        options: [
          {
            label: "외부 헤드리스 CMS 도입",
            verdict: "기각",
            reason:
              "콘텐츠만을 위해 별도 서비스와 데이터 저장소를 하나 더 운영해야 합니다. 기존 서비스 데이터와 연결하려면 결국 연동 계층을 따로 만들어야 하고, 운영·인증·권한이 두 곳으로 갈라집니다.",
          },
          {
            label: "기존 스택 위에 직접 구축",
            verdict: "채택",
            reason:
              "쓰던 서버·DB·인증·스토리지를 그대로 재사용하면 추가로 운영할 인프라가 없습니다. 새 서비스를 익히고 붙이는 시간보다, 익숙한 스택 위에서 바로 만드는 쪽이 빠르다고 봤습니다.",
          },
        ],
        outcome:
          "범용 CMS를 목표로 하지 않고 당시 운영에 필요한 범위만 담아 가볍게 만들었습니다. 새로 도입한 인프라 없이, 설계부터 운영팀 인수까지 1개월에 마쳤습니다.",
      },
    ],
    challenges: [
      {
