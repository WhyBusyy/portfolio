...
export const projects: ProjectData[] = [
  ...
  // ─── 제품 개발 ───
  {
    ...
    overview: {
      lead: "상품과 판매처가 1:1로 묶여 있던 구조를 상품 계층 3단으로 분리하면서, 검색·벤더·어드민 화면 3종을 구현하고 API 명세와 벤더 전용 서버 엔드포인트까지 직접 설계했습니다.",
      detail:
        "전문의약품은 같은 약이라도 규격과 판매처가 여러 갈래로 갈라지는데, 기존 데이터 모델은 상품과 판매처가 1:1로 묶여 있어 이를 표현할 수 없었습니다. 상품 계층을 세 단계로 분리하고, 그에 맞춰 약국이 쓰는 검색 화면, 업체가 상품을 등록·수정하는 화면, 관리자가 단계별로 승인하는 화면을 새로 만들었습니다. 화면 구현과 함께 응답 필드·정렬 규칙까지 담은 API 명세를 작성하고, 벤더 전용 서버 엔드포인트도 직접 구현했습니다.",
    },
    screenshots: [],
    challenges: [
      {
        ...
        after: [
          ...
          "검색·입력·선택·설정 단계를 공용 스텝 컴포넌트로 분리해 조립식으로 구성",
      ...
      },
      {
        ...
        before:
          "계층별 정보와 판매처 정보를 각각 조회해 화면 하나를 그리는 데 네 번의 왕복이 필요",
        after:
      ...
      },
      {
        ...
        before:
          "업체가 선택한 상위 계층이 잘못된 경우, 기존 연결을 끊고 새로 잇는 과정에서 중간에 실패하면 데이터가 어중간한 상태로 남을 위험",
        after: [
      ...
      },
      {
        ...
        before:
          "상위 계층을 여러 업체가 공유하므로, 한 업체의 수정이 같은 계층을 쓰는 다른 업체까지 영향을 주는 구조",
        after:
          "수정 허용 조건을 서버 가드로 검증하고, 프론트의 편집 가능 여부를 서버 규칙과 1:1로 맞춘 뒤 잠긴 경우 이유를 배너로 안내",
      },
      {
        title: "필터 조합과 뒤로가기",
        before:
