...
export const projects: ProjectData[] = [
  ...
  // ─── 제품 개발 ───
  {
    ...
    overview: {
      lead: "담당자 변경과 디자인 리뉴얼이 겹치면서 페이지와 컴포넌트마다 적용된 스타일 기준이 달랐습니다. 디자인 측이 정의한 토큰 체계를 코드로 구현하고 공용 컴포넌트로 화면 전반을 통일한 뒤, 정합 규칙을 만들어 이후에도 디자인 시스템을 지키며 유지보수할 수 있게 했습니다.",
      detail:
        "3가지 문제가 있었습니다. 페이지와 컴포넌트마다 레거시 스타일과 현행 스타일이 갈려 있었고, 공용 컴포넌트는 일부 화면에서만 쓰였고, 색은 의미 토큰 대신 프리미티브 팔레트를 직접 참조하고 있었습니다. 담당자 변경과 디자인 리뉴얼이 겹치면서 적용이 일부 화면에만 이뤄진 채 누적된 상태였습니다. 디자인 측이 정리한 역할 기반 토큰 체계를 코드로 구현해 화면 전반에 적용하고, 남아 있던 직접 마크업을 공용 컴포넌트로 대체했습니다. 이후에도 기준을 따라갈 수 있도록 정합 규칙을 만들어 코드베이스에 남겼습니다.",
    },
    screenshots: [],
    challenges: [
      {
        title: "페이지·컴포넌트마다 적용된 스타일 기준이 다름",
        before: [
          "담당자 변경과 리뉴얼이 겹쳐 이전 기준과 새 기준이 코드에 함께 남음",
          "새 기준이 적용된 페이지와 그대로인 페이지가 나뉘어, 같은 버튼이 페이지마다 다르게 보이는 상태",
        ],
        after: [
