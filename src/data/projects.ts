...
export const projects: ProjectData[] = [
  ...
  // ─── 제품 개발 ───
  {
    ...
    overview: {
      lead: "디자인 담당자 교체와 부분 리뉴얼이 겹쳐 레거시와 현행 스타일이 한 화면에 섞여 있었습니다. 색상 팔레트까지만 구현돼 있던 디자인 시스템에 의미 기반 토큰 계층을 정의하고, 공용 컴포넌트로 화면 전반을 통일한 뒤 다시 무너지지 않도록 규칙으로 고정했습니다.",
      detail:
        "문제가 세 가지 겹쳐 있었습니다. 레거시 스타일과 현행 스타일이 산재했고, 공용 컴포넌트가 일부 화면에서만 쓰였고, 색은 의미 토큰이 아니라 프리미티브 팔레트를 그대로 가져다 쓰고 있었습니다. 디자인 시스템을 담당한 웹디자이너가 한 번 교체됐고, 새로 온 담당자가 팔레트 계층까지만 구현한 상태에서 일부 페이지만 리뉴얼이 진행된 결과였습니다. 그래서 이미 있는 시스템을 적용하는 일이 아니라, 미완성 상태를 먼저 완성하는 일부터 시작해야 했습니다.",
    },
    screenshots: [],
    challenges: [
      {
        ...
        before: [
          "디자인 시스템 담당자가 교체되면서 이전 기준과 새 기준이 코드에 함께 남음",
          "새 기준으로 리뉴얼된 페이지와 그대로인 페이지가 섞여, 같은 버튼이 화면마다 다르게 보이는 상태",
        ],
        after: [
          "리뉴얼 기준을 단일 기준으로 확정하고, 남아 있던 레거시 스타일 호출부를 화면 전반에서 정리",
          "약국 화면과 벤더 관리자 화면을 같은 기준으로 맞춰 서비스 전체의 시각적 일관성 확보",
        ],
      },
      {
        title: "디자인 시스템에 의미 계층이 없었음",
        before: [
          "새 디자인 시스템이 색상 팔레트 목록까지만 구현된 상태로 넘어옴",
          "그래서 화면에서는 팔레트 값이나 색상 코드를 직접 지정했고, 같은 의미의 색이 화면마다 다른 값으로 들어감",
          "디자인 변경 요청이 오면 어디를 고쳐야 하는지 특정할 수 없는 구조",
        ],
        after: [
          "텍스트·배경·테두리·아이콘·인터랙션 등 역할별 의미 계층을 CSS 변수로 정의하고 Tailwind에 매핑",
          "투명도 조합까지 지원하도록 채널 포맷으로 정의해, 같은 토큰으로 옅은 배경까지 처리",
          "팔레트 값과 색상 코드 직접 지정을 금지하고 의미 토큰만 쓰도록 전환",
          "타이포도 제목·라벨·본문 체계로 정리해 임의 클래스 사용을 대체",
        ],
      },
      {
      ...
      {
        title: "한 번 맞추는 것보다 유지하는 게 어려움",
        before:
          "이번에 전부 맞춰도, 다음 기능 개발에서 레거시 스타일을 한 곳만 다시 들이면 화면 간 정합이 무너지는 구조",
        after: [
          "UI 수정 시 지켜야 할 규칙을 코드베이스 문서에 필수 항목으로 고정 — 공용 컴포넌트 우선 재사용, 의미 토큰 우선, 팔레트·색상 코드 직접 지정 금지, 타이포는 정해진 체계만",
          "토큰 정의 위치와 매핑 위치를 함께 명시해 다음 사람이 찾아갈 수 있게 함",
        ],
      },
    ],
    metrics: [
      {
        value: "의미 계층 신설",
        change: "팔레트만 있던 상태 → 역할 기반 토큰",
        label: "디자인 시스템의 빠진 계층을 정의하고 Tailwind에 매핑",
      },
      {
        value: "전면 적용",
      ...
      {
        value: "규칙 고정",
        change: "레거시 스타일 재유입 차단",
        label: "이후 UI 수정 시 정합 규칙을 코드베이스에 명시",
      },
    ],
  },
  {
