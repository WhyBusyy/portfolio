...
export const projects: ProjectData[] = [
  ...
  // ─── 제품 개발 ───
  {
    ...
    overview: {
      lead: "화면마다 색과 글자 크기가 조금씩 다르던 서비스에 시맨틱 토큰 297개와 공용 컴포넌트로 이뤄진 디자인 시스템을 만들고, 약국·벤더 관리자 화면 전반에 적용했습니다. 적용에서 끝내지 않고 레거시 클래스가 다시 들어오지 못하도록 규칙을 코드베이스에 못 박았습니다.",
      detail:
        "서비스가 커지는 동안 화면별로 색과 타이포가 조금씩 달라져, 같은 버튼인데 페이지마다 다르게 보이는 상태였습니다. 디자인 측 가이드에 맞춰 색·간격·타이포·상태를 시맨틱 토큰으로 정의하고 Tailwind에 매핑한 뒤, 버튼·탭·입력·뱃지 같은 공용 컴포넌트를 정리했습니다. 브랜드 색과 로고 교체까지 이 작업에 포함됐습니다.",
    },
    screenshots: [],
    challenges: [
      {
        title: "화면마다 조금씩 다른 색과 글자 크기",
        before: [
          "색을 #hex로 직접 쓰거나 프리미티브 팔레트를 그대로 가져다 쓴 곳이 산재",
          "같은 의미의 회색이 화면마다 다른 값이라, 디자인 변경 요청이 오면 어디를 고쳐야 하는지 알 수 없음",
        ],
        after: [
          "의미 기반 토큰 297개를 CSS 변수로 정의 — 텍스트·배경·테두리·인터랙션·요소·아이콘·간격을 역할 이름으로 분리",
          "RGB 채널 포맷으로 정의해 Tailwind에서 투명도 조합까지 지원하도록 매핑",
          "#hex 직접 지정과 프리미티브 클래스 사용을 금지 규칙으로 명시",
        ],
      },
      {
        title: "같은 버튼을 화면마다 새로 마크업",
        before:
          "버튼·탭·입력·체크박스·뱃지를 페이지마다 직접 만들어, 한 곳을 고쳐도 나머지가 그대로 남는 구조",
        after: [
          "공용 컴포넌트로 정리하고 기존 호출부를 전부 대체 (Button, CategoryButton, TabButton, SelectInput, RadioButton, Checkbox, Badge 등)",
          "상품 카드는 크기 × 방향 × 상태 조합을 하나의 컴포넌트로 묶고, 상태 판정 로직을 분리해 단위 테스트로 고정",
        ],
      },
      {
        title: "구 타이포 클래스가 코드 전반에 박혀 있음",
        before:
          "글자 크기와 굵기를 임의 클래스로 쓰던 흔적이 화면 전반에 남아 있어, 타이포 체계를 바꾸면 어디가 깨지는지 예측 불가",
        after: [
          "제목·라벨·본문 × 크기 × 굵기 조합으로 타이포 클래스 54개를 정의",
          "구 클래스 사용처 222곳(82개 파일)을 일괄 마이그레이션",
        ],
      },
      {
        title: "정합을 한 번 맞추는 것보다 유지하는 게 어려움",
        before:
          "이번에 전부 맞춰도, 다음 기능 개발에서 레거시 클래스를 한 곳만 다시 들이면 화면 간 정합이 무너지는 구조",
        after: [
          "UI 수정 시 지켜야 할 DS 규칙을 코드베이스 문서에 필수 항목으로 고정 — 컴포넌트 우선 재사용, 시맨틱 토큰 우선, 하드코딩·프리미티브 금지, 타이포는 DS 클래스만",
          "토큰 정의 위치와 Tailwind 매핑 위치를 함께 명시해 다음 사람이 찾아갈 수 있게 함",
          "DS 변경으로 깨진 E2E 셀렉터를 QA 봇 스펙에서 함께 수정해 테스트가 계속 돌게 유지",
        ],
      },
    ],
    metrics: [
      {
        value: "297개",
        change: "색 · 간격 · 타이포 · 상태",
        label: "의미 기반 시맨틱 토큰 정의",
      },
      {
        value: "222곳",
        change: "82개 파일",
        label: "구 타이포 클래스 → DS 클래스 마이그레이션",
      },
      {
        value: "규칙 고정",
        change: "레거시 클래스 재유입 차단",
        label: "이후 UI 수정 시 DS 정합을 필수 규칙으로 문서화",
      },
    ],
  },
  {
