...
export const projects: ProjectData[] = [
  // ─── 품질 · 자동화 ───
  {
    ...
    overview: {
      lead: "PR마다 스모크·단위·E2E 94개 케이스를 자동 실행하고, 변경 diff를 분석해 Playwright 테스트를 AI로 생성·검증까지 하는 QA 봇을 설계·구축했습니다. 5개월간 CI에서 170회 실행되며 팀의 PR 게이트로 정착했습니다.",
      detail:
        "릴리스마다 사람이 손으로 화면을 눌러보는 QA가 병목이었습니다. 역할 5종(약국·업체·병의원·영업사원·비업체영업) 로그인 fixture를 만들고 고정 스위트를 PR 게이트로 세운 뒤, 변경된 코드에 대한 테스트는 AI가 생성하도록 파이프라인을 붙였습니다. 다만 AI가 만든 테스트를 그대로 신뢰할 수는 없기에, 문법 검증·DB 읽기 전용 강제·비용 로깅 같은 검증 장치를 함께 설계했습니다.",
    },
    screenshots: [],
    challenges: [
      {
        ...
        after: [
          "역할 5종 로그인 fixture 기반으로 스모크 18 · 단위 4 · E2E 72 = 94개 케이스 구성",
      ...
      },
      {
        ...
        before:
          "생성된 코드가 문법적으로 깨지거나, 테스트가 실제 데이터를 변조할 위험이 있는 상태",
        after: [
          "생성 코드 TypeScript 문법 검증 후 실패 시 에러 컨텍스트를 넣어 1회 재시도",
          "네트워크 계층에서 DB 쓰기를 하드 차단해 AI 생성 테스트가 데이터를 변조하지 못하도록 강제",
      ...
      },
      {
        ...
        before:
          "전체 스위트를 순차 실행하면 PR 피드백이 늦고, 테스트 전용 인프라가 상시 과금되는 구조",
        after: [
          "워커 3 병렬 + 빌드 해시 스킵 + 빌드 병렬화로 웜 캐시 기준 8~12분까지 단축",
          "전용 EC2를 기본 정지 상태로 두고 실행 시점에만 기동·종료하는 온디맨드 방식으로 운용",
          "CI의 정적 AWS 키를 제거하고 OIDC role assume으로 전환, 로그인 비밀번호는 CI로 전달하지 않고 인스턴스가 직접 렌더",
        ],
      },
    ],
    metrics: [
      ...
      },
      {
        value: "read-only",
        change: null,
        label: "AI 생성 테스트의 DB 변조를 네트워크 계층에서 차단",
      },
    ],
  },
  {
    ...
    period: "2026년 6월",
    role: "Developer",
    techStack: ["GitHub Actions", "GitHub App", "Bash", "Git"],
    overview: {
      lead: "운영 브랜치의 핫픽스를 개발 브랜치로 손수 옮기고 충돌을 푸는 상시 업무를, 보조 CI 3종으로 자동화했습니다. 도입 첫 주 동기화 21회 중 17회가 사람 손 없이 머지됐습니다.",
      detail:
        "운영(main)에 들어간 핫픽스를 개발(step3x)로 주기적으로 동기화해야 했는데, 그때마다 충돌이 생기고 매번 수동으로 풀어야 했습니다. 동기화가 밀리면 두 브랜치가 벌어지고, 잘못된 베이스에서 분기한 브랜치 때문에 미완성 기능이 운영에 노출될 위험도 있었습니다. 자동 머지·사전 경고·베이스 검증 세 층으로 나눠 해결했습니다.",
    },
    screenshots: [],
    challenges: [
      {
        title: "핫픽스 동기화가 상시 수동 업무",
        before: [
          "운영 핫픽스를 개발 브랜치로 주기적으로 직접 머지",
          "동기화 때마다 충돌 발생 → 매번 수동으로 해소하는 반복 비용",
          "동기화가 밀리면 두 브랜치의 격차가 누적",
        ],
        after: [
          "운영 브랜치에 push가 있을 때마다 개발 브랜치로 forward-merge하는 워크플로 구축",
          "충돌이 없으면 봇이 직접 머지, 충돌이 나면 해소용 PR을 자동 생성해 사람이 그 PR에서만 개입",
          "GitHub App 토큰으로 보호 규칙을 우회하고, concurrency로 직렬화해 동시 실행 시 꼬임 방지",
          "사람이 이미 해소 중인 PR은 건드리지 않도록 보호 처리",
        ],
      },
      {
        title: "충돌을 머지 시점에야 알게 됨",
        before:
          "운영 PR을 올릴 때는 문제가 없다가, 나중에 동기화 단계에서 충돌이 드러나 뒤늦게 대응",
        after:
          "운영 PR 단계에서 개발 브랜치와의 충돌 가능성을 미리 알려주는 검사를 추가 (블로킹하지 않고 알림만)",
      },
      {
        title: "잘못된 베이스에서 분기한 브랜치",
        before:
          "개발 브랜치 전용 커밋이 운영 PR에 섞여 들어가면 미완성 기능이 운영에 노출될 수 있는 구조",
        after:
          "운영 PR에 개발 전용 커밋이 포함됐는지 검사하는 워크플로를 required check로 지정해 머지 자체를 차단",
      },
    ],
    metrics: [
      {
        value: "17/21회",
        change: "도입 첫 주",
        label: "사람 개입 없이 자동 머지된 동기화",
      },
      { value: "3종", change: null, label: "자동 머지 · 사전 경고 · 베이스 검증 CI" },
      {
        value: "자동 격리",
        change: null,
        label: "충돌 시에만 해소 PR 생성 → 개입 범위 최소화",
      },
    ],
  },
  {
  ...
  // ─── 아키텍처 · 성능 ───
  {
    ...
    overview: {
      lead: "약 600개 파일 규모의 프론트엔드 구조를 재설계했습니다. 후보 4안을 비교한 뒤 의존성을 전수조사해 교과서적인 정답(완전한 Vertical Slice)을 근거와 함께 기각하고, 하이브리드 구조를 택했습니다.",
      detail:
    ...
    challenges: [
      ...
      },
      {
        ...
        after: [
          ...
          "이동 과정에서 미사용 파일 9개와 미사용 라우트 7개를 함께 정리 (145 → 138 페이지)",
        ],
      },
      {
    ...
    metrics: [
      {
        value: "600+",
        change: "145 → 138 페이지 정리",
        label: "마이그레이션 대상 TS/TSX 파일",
      },
      { value: "917회", change: null, label: "아키텍처 검증 CI 누적 실행" },
      {
        value: "0 errors",
  ...
  // ─── 제품 개발 ───
  {
    ...
    overview: {
      lead: "'상품 1 : 판매처 1' 구조를 '모상품 → 규격상품 → 판매설정' 3단 계층으로 전환하면서, 검색·벤더·어드민 화면 3종을 구현하고 API 명세와 벤더 전용 서버 엔드포인트까지 직접 설계했습니다.",
      detail:
        "전문의약품을 취급하려면 같은 약이라도 제조사·규격·판매처가 각각 달라지는 구조를 담아야 했는데, 기존 데이터 모델은 상품과 판매처가 1:1로 묶여 있어 이를 표현할 수 없었습니다. 계층을 셋으로 나누고, 그에 맞춰 약국이 쓰는 검색 화면, 업체가 상품을 등록·수정하는 화면, 관리자가 단계별로 승인하는 화면을 새로 만들었습니다. 화면 구현과 함께 응답 필드·정렬 규칙까지 담은 API 명세를 작성하고, 벤더 전용 서버 엔드포인트도 직접 구현했습니다.",
    },
    screenshots: [],
    challenges: [
      {
        title: "등록·수정·승인 모달이 6개로 분산",
        before: [
          "신규 등록, 기존 상품 선택, 규격 선택, 정보 수정, 승인 등 흐름마다 모달이 따로 존재",
          "같은 검색·선택 UI가 모달마다 중복 구현되어 한 곳을 고치면 나머지가 어긋남",
        ],
        after: [
          "mode prop으로 흐름을 구분하는 단일 통합 모달(9개 모드)로 재설계",
          "모상품 검색·모상품 입력·규격 선택·판매설정 입력 등을 공용 스텝 컴포넌트로 분리해 조립식으로 구성",
          "어드민에서 먼저 완성한 뒤 같은 컴포넌트를 벤더 화면으로 포팅해 양쪽 UI 일관성 확보",
        ],
      },
      {
        title: "상세 진입 시 API를 4번 호출",
        before:
          "판매설정·규격상품·모상품·판매처 정보를 각각 조회해 화면 하나를 그리는 데 네 번의 왕복이 필요",
        after:
          "계층 전체를 한 번에 내려주는 단일 조회 엔드포인트를 설계해 호출을 4회에서 1회로 축소하고, 하위 섹션은 개별 조회 대신 전달받은 데이터를 사용하도록 변경",
      },
      {
        title: "여러 관리자가 같은 상품을 동시에 편집",
        before:
          "승인 담당자가 여럿이라, 한 사람이 보고 있는 사이 다른 사람이 바꾼 내용을 모른 채 덮어쓸 수 있는 구조",
        after: [
          "조회 시점의 최종 수정 시각을 함께 받아 수정 요청에 되실어 보내는 낙관적 잠금 방식 도입",
          "서버는 그 시각이 현재 값과 일치할 때만 반영하고, 변경 감지 시 프론트에서 안내 모달을 띄우고 이동을 차단",
        ],
      },
      {
        title: "승인자가 다른 상품으로 재매핑할 때의 원자성",
        before:
          "업체가 선택한 모상품·규격이 잘못된 경우, 기존 연결을 끊고 새로 잇는 과정에서 중간에 실패하면 데이터가 어중간한 상태로 남을 위험",
        after: [
          "'재연결 → 기존 거절' 순서를 고정해, 앞 단계가 실패하면 원래 상태가 그대로 남아 그대로 재시도할 수 있도록 설계",
          "뒤 단계가 실패하면 '부분 성공'을 사용자에게 그대로 노출하고 관련 캐시를 무효화해 실제 상태를 숨기지 않음",
          "이미 처리한 단계는 세션 내에서 추적해, 뒤로 돌아가 다시 눌러도 서버에 중복 요청하지 않도록 처리",
        ],
      },
      {
        title: "업체의 수정이 다른 업체 데이터에 전파",
        before:
          "상위 계층을 여러 업체가 공유하므로, 한 업체가 모상품 정보를 고치면 같은 계층을 쓰는 다른 업체까지 영향을 받는 구조",
        after: [
          "상위 계층이 해당 업체 전속일 때만 수정을 허용하는 체인 검증을 서버 가드로 구현",
          "프론트는 상태별 편집 가능 여부를 표로 정의해 서버 규칙과 1:1로 맞추고, 잠긴 경우 이유를 배너로 안내",
        ],
      },
      {
        title: "필터 조합과 뒤로가기",
        before:
          "카테고리 3단 필터를 걸어둔 뒤 상세로 들어갔다 돌아오면 조건이 초기화되는 문제",
        after:
          "필터 상태와 URL 쿼리를 양방향 동기화하고 초기화 시 관련 쿼리를 함께 제거, 검색 결과는 무한 스크롤로 처리하며 구매 이력이 있는 상품을 우선 노출하도록 정렬",
      },
    ],
    metrics: [
      {
        value: "1:N:M",
        change: "상품 1 : 판매처 1 → 3단 계층",
        label: "모상품 → 규격상품 → 판매설정 구조 전환",
      },
      {
        value: "6 → 1",
        change: "9개 모드 통합",
        label: "등록·수정·승인 모달 통합",
      },
      {
        value: "4 → 1",
        change: null,
        label: "상세 진입 시 API 호출 횟수",
      },
    ],
  },
  {
