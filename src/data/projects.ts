...
export const projects: ProjectData[] = [
  ...
  // ─── 제품 개발 ───
  {
    ...
    overview: {
      lead: "담당자 변경과 디자인 리뉴얼이 겹치면서 페이지와 컴포넌트마다 적용된 스타일 기준이 갈려 있었습니다. 디자인 측이 정의한 토큰 체계를 코드로 구현하고, 따로 마크업돼 있던 요소를 공용 컴포넌트로 대체한 뒤, 정합 규칙을 만들어 이후에도 디자인 시스템을 지키며 유지보수할 수 있게 했습니다.",
      detail:
        "3가지 문제가 있었습니다. 이전 기준과 현행 기준으로 만든 화면이 함께 남아 있었고, 공용 컴포넌트가 일부에서만 쓰이고 나머지는 같은 요소를 따로 마크업해 구현돼 있었고, 색은 의미 토큰 대신 프리미티브 팔레트를 직접 참조하고 있었습니다. 담당자 변경과 디자인 리뉴얼이 겹치면서 적용이 일부에만 이뤄진 채 누적된 상태였습니다. 디자인 측이 정리한 역할 기반 토큰 체계를 코드로 구현해 약국·벤더 관리자 화면 전반에 적용하고, 따로 마크업돼 있던 요소를 공용 컴포넌트로 대체했습니다. 이후에도 기준을 따라갈 수 있도록 정합 규칙을 코드베이스에 남겼습니다.",
    },
    screenshots: [],
    challenges: [
      {
        ...
        after: [
          "현행 기준을 단일 기준으로 확정하고, 남아 있던 레거시 스타일 호출부를 약국·벤더 관리자 화면 전반에서 정리",
